...
static void	rotate_both(stack_node **a, stack_node **b, stack_node *cheapest_node) //Define a function that rotates both the top `a` and `b` nodes to the bottom of their stacks, if it's the cheapest move
{
...
static void	rev_rotate_both(stack_node **a, stack_node **b, stack_node *cheapest_node) //Define a function that rotates both the bottom `a` and `b` nodes to the top of their stacks, if it's the cheapest move
{
...
static void	move_a_to_b(stack_node **a, stack_node **b) //Define a function that prepares the cheapest nodes on top of the stacks for pushing `a` nodes to stack `b`, until there are three nodes left in `a`
{
	stack_node	*cheapest_node; //To store the pointer to the cheapest `a` node
...
static void	move_b_to_a(stack_node **a, stack_node **b) //Define a function that prepares `b`'s target `a` nodes for pushing all `b` nodes back to stack `a` 
{
	prep_for_push(a, (*b)->target_node, 'a'); //Ensure `b`'s target `a` node is on top of the stack
	pa(a, b, 0); 
}

static void	min_on_top(stack_node **a) //Define a function that moves the smallest number to the top
{
	while ((*a)->data != smallest_node(*a)->data) //As long as the smallest number is not at the top
	{
		if (smallest_node(*a)->above_median == 1) //Rotate or reverse rotate according to the position of the node on the median
			ra(a, 0);
		else
			rra(a, 0);
	}
...
void	sort_stacks(stack_node **a, stack_node **b) //Define a function that sorts stack `a` if it has more than 3 nodes
{
	int	len_a; //To store the length of stack `a`

	len_a = stack_len(*a);
	if (len_a-- > 3 && (is_stack_sorted(*a) == 0)) //If stack `a` has more than three nodes and aren't sorted
		pb(b, a, 0);
	if (len_a-- > 3 && (is_stack_sorted(*a) == 0)) //If stack `a` still has more than three nodes and aren't sorted
		pb(b, a, 0);
	while (len_a-- > 3 && (is_stack_sorted(*a) == 0)) //If stack `a` still has more than three nodes and aren't sorted
	{
