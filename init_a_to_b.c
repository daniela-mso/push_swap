...
void	current_index(stack_node *stack)
{
	...
	if (stack == NULL) //Check for am empty stack
		return ;
	median = stack_len(stack) / 2; //Calculate the median by dividing the length of a stack by 2
	while (stack != NULL) //Loop through all the nodes until the end of the stack is reached
	{
...
static void	set_target_a(stack_node *a, stack_node *b)//Find `a` node's target in stack `b`
{
	stack_node	*current_b; //To store the pointer to the current node in stack `b` and iterate through each node following
	stack_node	*target_node; //To store the pointer to the target node in stack `b`
	long			best_match_index; //In this case, the best match indexe stores the value of the "closest smaller number" so far

	while (a != NULL) //As long as we have nodes in stack `a`
	{
		...
		if (best_match_index == LONG_MIN) //Check if the LONG_MIN hasn't changed, it means we haven't found anything smaller
			a->target_node = biggest_node(b); //If so, find the biggest `nbr` and set this as the target node
		else
...
static void	cost_analysis_a(stack_node *a, stack_node *b) //Define a functio that analyses the cost of the `a` node along with it's target `b` node, which is the sum of the number of instructions for both the nodes to rotate to the top of their stacks
{
	...
	while (a) //Loop through each node until the end of the stack is reached
	{
		...
		if ((a->above_median == 0)) //Check if the above_median data is false, meaning it is below median
			a->push_cost = len_a - (a->index); //If so, update `a` node's push cost to the stack's length - index
		if (a->target_node->above_median == 1) //Check if `a` node's target node `b` has a "true" above median attribute, meaning the target `b` node is above median
...
void	set_cheapest(stack_node *stack) //Define a function that sets a node's `cheapest` attribute as `true` or `false`
{
	long			cheapest_value; //To store the value of the cheapest node so far
	stack_node	*cheapest_node; //To store a pointer to the cheapest node so far

	if (stack == NULL) //Check for an empty stack
		return ;
	cheapest_value = LONG_MAX; //Assign the biggest `long` as the cheapest value so far
	while (stack != NULL) //Loop through every node until the end of the stack is reached, and we find the cheapest node
	{
...
void	init_nodes_a(stack_node *a, stack_node *b) //Define a function that combines all the functions needed to prepare stack `a`, ready for our pushing and sorting. These functions set the data inside the node's structure
{
	current_index(a);
	current_index(b);
	set_target_a(a, b);
